...
export class DshHost extends EventEmitter {
  constructor({
    ...
    if (Boolean(moduleLoaderPath) !== Boolean(runtimeArchivePath)) {
      throw new TypeError('DshHost requires moduleLoaderPath and runtimeArchivePath together');
    }
  ...
  #startInternal(workspace) {
    ...
    let child;
    try {
      const runtimeArgs = ['--expose-internals'];
      if (this.moduleLoaderPath) {
        runtimeArgs.push('--import', pathToFileURL(this.moduleLoaderPath).href);
      }
      runtimeArgs.push(
        this.entryPath,
        'web',
      ...
      const childEnvironment = { ...process.env, ELECTRON_RUN_AS_NODE: '1' };
      if (this.runtimeArchivePath) {
        childEnvironment.DSH_DESKTOP_RUNTIME_ARCHIVE = this.runtimeArchivePath;
        childEnvironment.NODE_PATH = [
          path.join(this.runtimeArchivePath, 'node_modules'),
          childEnvironment.NODE_PATH,
        ].filter(Boolean).join(path.delimiter);
      }
      child = this.spawnImpl(this.executable, runtimeArgs, {
        cwd: workspace,
